...
int		**tab_solved(int **tab, int i, int j, int fw_bw);
...
int		**tab_solved(int **tab, int i, int j, int fw_bw)
{
	while (i >= 0 && i < 4)
	{
		while (j >= 0 && j < 4)
		{
			...
			{
				tab[i][j] = tab[i][j] + 1;
				fw_bw = check_case(tab, tab[i][j], i, j, 4);
				if (fw_bw == 1) // Cas ou la valeur de tab[i][j] est bonne
					break ;
			}
			if (tab[i][j] >= 4 && fw_bw != GO_NEXT)
				tab[i][j] = 0;
			if (tab[0][0] == 0)
...
void	my_rush(int **arg, int p)
{
	...
	tab = create_initial_tab(p);
	while (!check_if_ok(arg, tab, p) || tab[0][0] == 0)
	{
	...
	print(tab, p);  // On sort de la boucle pq check_ok = 1 => print le tableau
}
